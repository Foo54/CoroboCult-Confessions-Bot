...
from corobot.config import MOD_ROLE_ID, BOT_COLOR
...
async def submit_confession(
	...
	confessions.execute(
		...
		query="""INSERT INTO confessions(author_id) VALUES (:AUTH)""",
	)
	_id: int = confessions.query_single_item(
		query="""SELECT id from confessions ORDER BY id DESC LIMIT 1"""
	)["id"]  # pyright: ignore[reportOptionalSubscript]
	...
	embed.set_footer(
		text=f"Made by CoroboCult Mod Team", icon_url=interaction.client.user.avatar.url
	...
	confessions.execute(
		params,
		query="""INSERT INTO confession_data(id, content, attachment_id) VALUES (:COID, :CONT, :ATCH)""",
	)
	...
	confessions.execute(
		{"COID": _id, "MEID": message.id},
		query="""UPDATE confessions SET message_id = :MEID WHERE id = :COID""",
	)
...
async def submit_reply(
	...
	data = confessions.query_single_item(
		parameters={"COID": _id}, query="""SELECT * FROM confessions WHERE id = :COID"""
	)
	...
	thread: discord.Thread = msg.thread or await msg.create_thread(
		name="Replies", reason="Confession Replies"
	)

	confessions.execute(
		{"COID": _id, "AUTH": interaction.user.id},
		query="""INSERT INTO replies (confession_id, author_id) VALUES (:COID, :AUTH)""",
	)

	rid = confessions.query_single_item(
		query="SELECT id FROM replies ORDER BY id DESC LIMIT 1"
	)["id"]  # pyright: ignore[reportOptionalSubscript]
	...
	confessions.execute(
		params,
		query="""INSERT INTO reply_data(id, content, attachment_id) VALUES (:REID, :CONT, :ATCH)""",
	)
	...
	confessions.execute(
		{"REID": rid, "MEID": message.id},
		query="""UPDATE replies SET message_id = :MEID WHERE id = :REID""",
	)


class QueryModal(discord.ui.Modal, title="Query Database"):
	query = discord.ui.Label(
		text="Query", component=discord.ui.TextInput(style=discord.TextStyle.paragraph)
	)
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		...
		query = (
			self.query.component.value.strip()
		...
		if not self.select:
			...
			await util.paginator(
				...
				title="Query Results",
				followup=True,
				iterator=confessions.query(
					query=query, size=self.size
	...
	async def on_error(self, interaction: discord.Interaction, error: Exception):
		if isinstance(error, app_commands.MissingRole):
			return await util.send_error_message(
				interaction, f"<@{MOD_ROLE_ID}> Permissions Needed", ephemeral=True
			)
...
class ParametersModal(discord.ui.Modal, title="Parameters"):
	def __init__(
		self, select: bool, code: str, size: int, parameters: typing.Sequence[str]
	):
		super().__init__()
		self.select = select
		self.code = code
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		parameters = {
			child.custom_id: child.value for child in self.walk_children()
		}  # pyright: ignore[reportAttributeAccessIssue]
		if not self.select:
			...
			await util.paginator(
				...
				title="Query Results",
				iterator=confessions.query(
					query=self.code, size=self.size, parameters=parameters
				),  # pyright: ignore[reportCallIssue, reportArgumentType]
				known_size=self.size if self.size != -1 else None,
				text=sql.rows_to_discord,
				items_per_page=25,
	...
	async def on_error(interaction: discord.Interaction, error: Exception):
		if isinstance(error, app_commands.CommandInvokeError):
			if isinstance(error.original, sql.MacroError):
				return await error.original.send_error_message(
...
class ConfessionView(discord.ui.View):
	...
	async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
		await interaction.response.send_modal(
			ConfessionModal(interaction.channel)
...
class ConfessionModal(discord.ui.Modal, title="Submit a Confession"):
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		await submit_confession(
			interaction,
			self.channel,
			self.confession.component.value,
			self.attachment.component.values,
		)  # pyright: ignore[reportAttributeAccessIssue]
		await interaction.response.send_message(
			f"Confession Submitted to {self.channel.mention}\n-# Please be aware that confession data is logged for moderation purposes",
			ephemeral=True,
		)
...
class ReplyModal(discord.ui.Modal, title="Reply to a Confession"):
	...
	number = discord.ui.Label(
		text="Reply Number",
		component=discord.ui.TextInput(
			required=False,
			placeholder="Leave blank to reply to the most recent confession",
		),
	)
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		...
		await submit_reply(
			interaction,
			int(self.number.component.value or self.num),
			self.confession.component.value,
			self.attachment.component.values,
		)  # pyright: ignore[reportAttributeAccessIssue]

		await interaction.followup.send(
			f"Confession Submitted to Confession #{self.number.component.value or self.num}",
			ephemeral=True,
		)  # pyright: ignore[reportAttributeAccessIssue]
...
class ConfessionsModCog(
	commands.GroupCog, name="confessmod", description="Moderation commands"
):
	...
	async def cog_app_command_error(
		...
		await util.send_error_message(
			interaction, f"Unexpected Failure! Please Report\n{error}", ephemeral=True
		)
		logger.error(error)
	...
	async def add_macro(self, interaction: discord.Interaction, name: str, code: str):
		...
		confessions_macro_manager.create_macro(
			name=name, author_id=interaction.user.id, code=code
		)
		await interaction.followup.send("Macro successfully created!")

	# VIEW MACROS
	@app_commands.command(
		name="view-macro", description="View all macros, or get data on a specific one"
	)
	@app_commands.describe(name="Get info on a specific macro")
	@app_commands.checks.has_role(MOD_ROLE_ID)
	async def view_macros(
		self, interaction: discord.Interaction, name: typing.Optional[str] = None
	):
		await interaction.response.defer()
		output = ""
		...
		if name:
			...
			if macro["parameters"]:  # pyright: ignore[reportOptionalSubscript]
				embed.add_field(
					name="Parameters",
					value=", ".join(
						[f"`{parameter}`" for parameter in macro["parameters"]]
					),  # pyright: ignore[reportOptionalSubscript]
				)
		await interaction.followup.send(embed=embed)

	@view_macros.autocomplete("name")
	async def command_autocomplete_view_macros(
		self, interaction: discord.Interaction, current: str
	) -> typing.List[app_commands.Choice[str]]:
		return [
			app_commands.Choice(name=value["name"], value=value["name"])
			for value in confessions_macro_manager.get_macros()
			if current in value["name"]
		][:25]
	...
	async def edit_macro(
		...
		await interaction.response.defer(ephemeral=True)
		confessions_macro_manager.edit_macro(
			name=name, new_name=new_name, editor=interaction.user.id, new_code=new_code
		)
		await interaction.followup.send("success")

	@edit_macro.autocomplete("name")
	async def edit_macro_autocomplete(
		self, interaction: discord.Interaction, current: str
	) -> typing.List[app_commands.Choice[str]]:
		return [
			app_commands.Choice(name=value["name"], value=value["name"])
			for value in confessions_macro_manager.get_macros()
			if current in value["name"] and value["author_id"] == interaction.user.id
		][:25]
	...
	async def delete_macros_autocomplete(
		self, interaction: discord.Interaction, current: str
	) -> typing.List[app_commands.Choice[str]]:
		return [
			app_commands.Choice(name=value["name"], value=value["name"])
	...
	async def use_macro(
		...
		if data["parameters"]:
			await interaction.response.send_modal(
				ParametersModal(select, data["code"], size, data["parameters"])
			)
		else:
			if not select:
	...
	async def use_macro_autocomplete(
		self, interaction: discord.Interaction, current: str
	) -> typing.List[app_commands.Choice[str]]:
		return [
			app_commands.Choice(name=value["name"], value=value["name"])
	...
	@app_commands.command(
		name="add-channel",
		description="Add a new channel to the confession channel list",
	)
	...
	@app_commands.command(
		name="remove-channel",
		description="Remove a channel from the confession channel list",
	)
	...
	async def remove_channel(
		...
		if str(channel.id) in VALID_CHANNELS:
			...
			await interaction.followup.send(
				f"{channel.mention} not in vaild channel list"
			)

	@app_commands.command(
		name="view-channels", description="View confession channel list"
	)
	@app_commands.checks.has_role(MOD_ROLE_ID)
	async def view_channels(self, interaction: discord.Interaction):
		await interaction.response.defer()
		await interaction.followup.send(
			"## Current Confessions Channel List\n"
			+ "\n".join(f"<#{id}>" for id in VALID_CHANNELS)
		)


class ConfessionsUserCog(
	commands.GroupCog, name="confessions", description="Anonymous Confession Commands"
):
	def __init__(self, bot):
		self.bot = bot
		super().__init__()
	...
	async def confess(
		self, interaction: discord.Interaction, channel: typing.Optional[str] = None
	):
		channel = channel or str(
			interaction.channel.id
		)  # pyright: ignore[reportOptionalMemberAccess, reportAssignmentType]
		if channel not in VALID_CHANNELS:  # pyright: ignore[reportOptionalMemberAccess]
			await interaction.response.send_message(
				"This is not a valid channel", ephemeral=True
			)
		else:
			await interaction.response.send_modal(
				ConfessionModal(interaction.guild.get_channel(int(channel)))
			)  # pyright: ignore[reportOptionalMemberAccess, reportArgumentType]

	@confess.autocomplete("channel")
	async def command_autocomplete_confess(
		self, interaction: discord.Interaction, current: str
	) -> typing.List[app_commands.Choice[str]]:
		return [
			app_commands.Choice(
			...
			for _channel in VALID_CHANNELS
			if current
			in (
